...
var listAlphabet = function(num) {
	...
	for (var i = 1; i <= 26; i++) {
		...
		table += '<td><button id="letter' + character + '">' + character + '</button></td>';
		if (!instructors.reduce(function(a,b){return new RegExp("(\\s" + character + "[A-Z])").test(b) || a},false)) {
	...
	if (num) {
		$("#filteredProfessors").html(instructors.reduce(function(a,b){return new RegExp("(\\s" + num + "[A-Z])").test(b) ?  a+"<li><button id='"+b.split(" ").join("")+"' onclick='addToCourseCart(\""+b+"\");'>"+b+"</button></li>" : a}, "<ul class='professorList'>") + "</ul>");
...
var addCartButton = function() {
	...
	$('html').on('click', function(e) {
		if (typeof $(e.target).data('original-title') == 'undefined' && !$(e.target).is('button')  && !$(e.target).parents().is('.popover.in')) {
